# References

## LLM Wiki

- [Andrej Karpathy's LLM Wiki gist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)
- [Pratiyush/llm-wiki](https://github.com/Pratiyush/llm-wiki)

Newswiki uses the same broad idea: compile raw information into a durable Markdown wiki. Newswiki adds an MCP-first agent startup protocol around the wiki.

## Search And Web Reading

- [SearXNG](https://github.com/searxng/searxng)

AgentSearch-style systems can wrap search engines, content extraction, caching, and scrubbing behind an agent-friendly API or MCP server.

## Deployment

- [Vercel](https://vercel.com/docs)

The web UI can be static JSON plus a lightweight frontend. Vercel is optional.
