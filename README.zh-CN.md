...
它解决的不是“怎么做一个新闻网站”，而是：当你的 Agent 开始做一件事之前，怎样先拿到真正有用的上下文：

1. 我以前知道什么？ -> Wiki MCP
2. 最近发生了什么？ -> Newsfeed MCP
3. 这个任务需要查工具/技能/工作流吗？ -> Capability MCP，按需使用

很多人都会做信息源、知识库、网页展示。Newswiki 的核心是 **上下文协议**：让 Agent 在规划和执行之前，先查询长期记忆和外部近期信息。Capability MCP 只在工具选择、工作流选择、MCP 配置、本地能力可用性这些任务里使用，不再强制每次开工都查。
